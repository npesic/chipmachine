...
// todo: yet another monolith that should better be chopped up
  byte __thiscall IXS__WAVEGEN__FUN_0x40adb0(WaveGen *obj, Local400 *floats100, uint numFloats100) {
    ...
    obj->buf256Ptr_0x20 = (double *) malloc(256);
    obj->buf256Ptr_0x24 = (double *) malloc(256);
...
#if !defined(EMSCRIPTEN) && !defined(LINUX)
      BufSFXI *cacheFileBuf = (BufSFXI *) _GlobalAlloc(0x1000, 0x1e00000); // this seems to be a memory leak in the original impl
#else
      if (!cacheFileBuf) {
...
#elif defined(LINUX)
    // LINUX: cache is kept in memory, no file save needed
    (*fileSFXI->vftptr_0x0->deleteObject)(fileSFXI);
#else
