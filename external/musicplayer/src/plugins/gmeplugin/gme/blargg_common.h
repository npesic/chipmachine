...
// Compatibility shim: blargg_ok was added in later releases; older code uses 0
#ifndef blargg_ok
	#define blargg_ok 0
#endif

// Compatibility shim: BLARGG_EXPORT was added in later releases
#ifndef BLARGG_EXPORT
	#define BLARGG_EXPORT
#endif

// Standard error strings used by newer gme files
#ifndef blargg_err_file_type
	#define blargg_err_file_type "Wrong file type for this emulator"
#endif
#ifndef BLARGG_ERR_CALLER
	#define BLARGG_ERR_CALLER "Caller error"
#endif

// dprintf: used in newer gme files for debug output — map to no-op.
// Must be done after stdio includes to avoid conflicting with POSIX dprintf(int fd,...).
// Files using dprintf() should include blargg_source.h last; the macro is set there.

// min: used in Sgc_Impl.cpp
#ifndef BLARGG_MIN_DEFINED
	#define BLARGG_MIN_DEFINED
	#include <algorithm>
	using std::min;
	using std::max;
#endif
