...
void SQT_Play(AYSongInfo &info)
{
    ...
    // SQT_Init() bails out early (leaving info.data == 0) when SQT_PreInit
    // rejects the module -- e.g. a corrupt .sqt, or a non-SQT file that only
    // matched by extension (a Quartet PSG ".sqt" reaches here because
    // ay_sys_detect falls back to the extension when no content-detector
    // claims the file). Every SQT_* macro dereferences info.data, so without
    // this guard the first access (--SQT.DelayCounter) segfaults on null and
    // takes down the whole host. Render silence instead of crashing.
    if(!info.data)
        return;
