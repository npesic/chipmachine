 ...
 //sample banks
  ...
  public:
    ...
    DivEngine():
      ...
      // [chipmachine local patch] filePlayerBuf is an array of POINTERS, so it
      // must be zeroed with sizeof(float*), not sizeof(float) -- the original
      // left the upper half uninitialized, and nextBuf()'s resize path then did
      // `delete[] filePlayerBuf[i]` on those garbage pointers (only benign when
      // the fresh-process heap happened to be zero). Report upstream.
