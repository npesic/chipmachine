...
song_load(song_t *song, const char *uri)
{
  ...
  if ( 0
       || (ecode = zz_vfs_open_uri(&vfs,uri))
       || (ecode = zz_vfs_read_exact(vfs,hd,16))
...
vset_load(vset_t *vset, const char *uri)
{
  ...
  if ( 0
       || (ecode = zz_vfs_open_uri(&vfs,uri))
       || (ecode = zz_vfs_read_exact(vfs,hd,222))
...
q4_load(vfs_t vfs, q4_t *q4)
{
  ...
  zz_assert( zz_vfs_tell(vfs) == 20 );

  ecode = E_SNG;
  if (q4->songsz < 16 + 12*4) {
    dmsg("invalid .4q song size (%lu) -- %s", LU(q4->songsz), zz_vfs_uri(vfs));
    goto error;
  }

  ecode = E_SET;
  if (q4->vset && q4->vsetsz < 222) {
    dmsg("invalid .4q set size (%lu) -- %s", LU(q4->vsetsz), zz_vfs_uri(vfs));
    goto error;
  }
  ...
  if (q4->song) {
    ...
    ecode = zz_vfs_seek(vfs, q4->songsz, ZZ_SEEK_CUR);
  ...
  if (q4->vset) {
    ...
    ecode = zz_vfs_seek(vfs, q4->vsetsz, ZZ_SEEK_CUR);
...
zz_load(play_t * P, const char * songuri, const char * vseturi, zz_u8_t * pfmt)
{
  ...
  do {
    ...
    ecode = zz_vfs_open_uri(&inp, songuri);
    if (ecode == E_OK)
      /* Read song header */
      ecode = zz_vfs_read_exact(inp, hd, 16);
    if (ecode != E_OK)
      break;
    ...
    if (!zz_memcmp(hd,"QUARTET",8)) {
      ...
      ecode = zz_vfs_read_exact(inp, hd+16, 4);
      if (E_OK != ecode)
	break;
