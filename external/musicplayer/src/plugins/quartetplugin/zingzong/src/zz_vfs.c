...
#ifdef NO_LOG
# define zz_vfs_emsg(DRI,ERR,FCT,ALT,OBJ) zz_nop
# else
static void
zz_vfs_emsg(const char * dri, int err, const char * fct,
...
zz_err_t
zz_vfs_register(zz_vfs_dri_t dri)
{
  int i = E_ERR;
  ...
  if (dri) {
    ...
    if (i != E_OK )
      (void) zz_vfs_emsg(dri->name,0,"register",0,0);
  }
  return E_ERR & i;
}

zz_err_t
zz_vfs_unregister(zz_vfs_dri_t dri)
{
  int i = E_ERR;
  ...
  if (dri) {
	...
	(void) zz_vfs_emsg(dri->name,0,"unregister",0,0);
	i = E_ERR;
      }
    }
  }
...
zz_vfs_new(const char * uri, ...)
{
  ...
  if (k < 0)
    ...
    va_list list;
    va_start(list,uri);
    vfs = drivers[k]->vnew(uri, list);
    va_end(list);
    if (vfs)
      vfs->err = 0;
    else
      (void)zz_vfs_emsg(drivers[k]->name,0,"new",0,uri);
  }
  return vfs;
}

zz_err_t
zz_vfs_open(vfs_t vfs)
{
  zz_err_t ecode;
  VFS_OR_ARG(vfs);
  ecode = vfs->dri->open(vfs);
  if (ecode != E_OK)
    (void) zz_vfs_emsg(vfs->dri->name,vfs->err,"open",0,zz_vfs_uri(vfs));
  vfs->pb_pos = vfs->pb_len = 0;
  return ecode;
}

void
zz_vfs_instance_del(vfs_t * pvfs)
{
...
zz_err_t
zz_vfs_close(vfs_t vfs)
{
  zz_err_t ecode;
  VFS_OR_ARG(vfs);
  ecode = vfs->dri->close(vfs);
  if (E_OK != ecode)
    zz_vfs_emsg(vfs->dri->name,vfs->err,"close", 0, zz_vfs_uri(vfs));
  vfs->pb_pos = vfs->pb_len = 0;
  return ecode;
}

zz_err_t
zz_vfs_push(vfs_t vfs, const void * ptr, zz_u8_t size)
{
  zz_err_t ecode = E_INP;
  VFS_OR_ARG(vfs);
  if (vfs->pb_len != vfs->pb_pos) {
    zz_vfs_emsg(vfs->dri->name,vfs->err,"push", "not empty", zz_vfs_uri(vfs));
  }
  else if (size > sizeof(vfs->pb_buf)) {
    zz_vfs_emsg(vfs->dri->name,vfs->err,"push", "overflow", zz_vfs_uri(vfs));
  }
  else {
...
zz_u32_t
zz_vfs_read(zz_vfs_t vfs, void * ptr, zz_u32_t size)
{
  if (!size || size == ZZ_EOF)
    return size;
  if (!ptr) vfs = 0;
...
zz_err_t
zz_vfs_read_exact(vfs_t vfs, void * ptr, zz_u32_t size)
{
  zz_err_t ecode;
  u32_t n;
  ...
  if (n == ZZ_EOF) {
    zz_vfs_emsg(vfs->dri->name, vfs->err,"read_exact", 0, zz_vfs_uri(vfs));
    ecode = E_INP;
  }
  else if (n != size) {
...
zz_err_t
zz_vfs_seek(vfs_t vfs, zz_u32_t pos, zz_u8_t set)
{
  zz_err_t ecode = E_INP;
  VFS_OR_ARG(vfs);
  ...
  if (vfs->pb_len != vfs->pb_pos) {
    vfs->err = E_ERR;
    zz_vfs_emsg(vfs->dri->name, vfs->err,"seek",
	     "push not empty", zz_vfs_uri(vfs));
    return E_INP;
  }
  ...
  ecode = vfs->dri->seek
    ...
    ;
  if (ecode)
    zz_vfs_emsg(vfs->dri->name,vfs->err,"seek","seek error", zz_vfs_uri(vfs));
...
vfs_seek_simul(vfs_t vfs, zz_u32_t pos, zz_u8_t set)
{
  ...
  default:
    zz_vfs_emsg(vfs->dri->name, vfs->err = ZZ_EINVAL,
	     "seek", "invalid whence", zz_vfs_uri(vfs));
    return E_666;
  }
...
zz_err_t
zz_vfs_open_uri(vfs_t * pvfs, const char * uri)
{
  zz_err_t ecode = E_ARG;
  vfs_t vfs = 0, ice = 0;
  if (likely(pvfs && uri)) {
    ecode = E_MEM;
    vfs = zz_vfs_new(uri,0);
    if (likely(vfs)) {
      ecode = zz_vfs_open(vfs);
      if (E_OK != ecode) {
	zz_vfs_instance_del(&vfs);
	zz_assert(vfs == 0);
      }
      else if (ice = zz_vfs_new("ice://", vfs, 0), !ice) {
	...
	zz_vfs_instance_del(&vfs);		/* close and destroy vfs */
	vfs = ice;		/* continue w/ ICE! vfs */
	ecode = zz_vfs_open(vfs);	/* never fails */
