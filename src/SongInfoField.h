...
struct SongInfoField : public Renderable
{
                  ...
                  uint32_t color = 0xffffffff)
    {
        ...
        fields[1] = std::make_shared<TextField>(font, "", x, y + 50 * sc,
                                                sc * 0.6, color);
        fields[2] = std::make_shared<TextField>(font, "", x, y + 100 * sc,
                                                sc * 0.4, color);
    }

    void setInfo(const SongInfo& info)
    {
        // LOGD("Set info '%s' '%s' '%s'", info.title, info.composer,
        // info.format);
        auto t = info.title;
    ...
    int getWidth(int no) const { return fields[no]->getWidth(); }
    ...
    const TextField& operator[](int i) const { return *fields[i]; }
};
